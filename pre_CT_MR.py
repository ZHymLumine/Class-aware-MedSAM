...
save_path_tr = join(npy_path, prefix, 'train')
save_path_ts = join(npy_path, prefix, 'test')
...
for name in tqdm(train_names):  # use the remaining 10 cases for validation
...
for name in tqdm(test_names):  # use the remaining 10 cases for validation
    image_name = name.split(gt_name_suffix)[0] + img_name_suffix
    gt_name = name
    gt_sitk = sitk.ReadImage(join(gt_path, gt_name))
    gt_data_ori = np.uint8(sitk.GetArrayFromImage(gt_sitk))
